...
use WPPM\Metabox\SimpleMetabox;
...
abstract class Base {
	...
	protected function createPostTypeLable () {
		...
		$labels = array(
			'name' => sprintf( _x( '%s', 'post type general name', WPPM_DOMAIN ), $plural ),
			'singular_name' => sprintf( _x( '%s', 'post type singular name', WPPM_DOMAIN ), $singular ),
			'add_new' => __( 'Add New', WPPM_DOMAIN ),
			'add_new_item' => sprintf( __( 'Add New %s', WPPM_DOMAIN ), $singular ),
			'edit_item' => sprintf( __( 'Edit %s', WPPM_DOMAIN ), $singular ),
			'new_item' => sprintf( __( 'New %s', WPPM_DOMAIN ), $singular ),
			'all_items' => sprintf( __( 'All %s', WPPM_DOMAIN ), $plural ),
			'view_item' => sprintf( __( 'View %s', WPPM_DOMAIN ), $singular ),
			'search_items' => sprintf( __( 'Search %s', WPPM_DOMAIN ), $plural ),
			'not_found' =>  sprintf( __( 'No %s found', WPPM_DOMAIN ), mb_strtolower( $plural, 'UTF-8') ),
			'not_found_in_trash' => sprintf( __( 'No %s found in Trash', WPPM_DOMAIN ), mb_strtolower( $plural, 'UTF-8') ),
			'parent_item_colon' => '',
			'menu_name' => sprintf( __( '%s', WPPM_DOMAIN ), $menu )
		);
		return $labels;
	}
